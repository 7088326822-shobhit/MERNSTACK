                            ...
                            <div className="hidden lg:block absolute min-w-[500px] min-h-[400px] overflow-hidden bg-slate-200 p-1 -right-[510px] top-0">
                                <div className="w-full h-full min-h-[400px] min-w-[500px] mix-blend-multiply scale-150" 
                                    style={{
                                        ...
                                        backgroundPosition: `${setZoomImage.x * 100}%  ${setZoomImage.y * 100}%`
                            ...
                            <div className="flex gap-2 lg:flex-col overflow-scroll scrollbar-none h-full">
                                {
                                    productImageLoading.map((el, index) => {
                                        return(
                                            <div className="h-20 w-20 bg-slate-200 rounded" key={"loadingImg"+index}></div>
                                        )
                                    })
                                }
                            </div>
                            ...
                            <div className="flex gap-2 lg:flex-col overflow-scroll scrollbar-none h-full">
                                {
                                    data?.productImage?.map((imgURl, index) => {
                                        return(
                                            <div className="h-20 w-20 bg-slate-200 rounded animate-pulse p-1" key={imgURl}>
                                                <img
                                                  src={imgURl}
                                                  className="w-full h-full object-scale-down
                                                  mix-blend-multiply cursor-pointer" onMouseEnter={() => handleMouseEnterProduct(imgURl)}
...

               {
                loading ? (
                 ...
                 <div className="flex items-center gap-2 text-2xl lg:text-3xl font-medium my-1 h-6 lg:h-8 animate-pulse w-full">
                    <p className="h-6 lg:h-8 text-slate-400 line-through bg-slate-200 w-full"></p>
                    <p className="h-6 lg:h-8 text-red-400 bg-slate-200 w-full"></p>
                 </div>
                 ...
                 <div>
                    <p className="text-align-600 font-medium my-1 h-6 lg:h-8 bg-slate-200 rounded animate-pulse w-full"></p>
                ...
                (
                 ...
                 <p className="bg-rose-200 text-red-600 px-2 rounded-full inline-block w-fit ">{data?.brandName}</p>
                 ...
                 <div className="flex items-center gap-2 text-2xl lg:text-3xl font-medium my-2">
                    <p className="text-slate-400 line-through">{displayCurrency(data?.selling)}</p>
                    <p className="text-red-400 ">{displayCurrency(data?.price)}</p>
                 </div>
                 ...
                 <div>
                    <p className="text-align-600 font-medium my-1">Description:</p>
